...
def reconstruct_dataset(
    degraded_file: str, output_file: str, reconstruction_model: str, config=None
) -> Dict[str, float]:
    """
    ...
    if reconstruction_model.startswith("stable_diffusion_2") and config:
        ...
        dataset_name = parse_degraded_filename(Path(degraded_file).name)["dataset"]
        runtime = resolve_sd2_runtime_settings(sd_settings, dataset_name, reconstruction_model, len(series))
        print(
            f"    Applying {reconstruction_model}... "
            f"(window={runtime['window_samples']}, context={runtime['context_samples']}, "
            f"image={runtime['image_size']}, steps={runtime['num_inference_steps']}, "
            f"guidance={runtime['guidance_scale']})"
        )
        reconstructed_series = model_func(series, **runtime)
    else:
...
Examples:
  ...
  python reconstruct_datasets.py --config config/my_config.yaml
    ...
    parser.add_argument("--filter-dataset", nargs="+", help="Only process specific datasets (e.g., boiler pump)")
    ...
    parser.add_argument(
        "--filter-iteration", nargs="+", type=int, help="Only process specific iterations (e.g., 1 2 3)"
    )
