...
def predict_with_darts_model(model, train_series: pd.Series, horizon: int) -> np.ndarray:
    ...
    for _ in range(horizon):
        pred = model.predict(n=1, series=current_ts)
  ...
  python 8_predict_datasets.py --models lstm xgboost holt_winters
    ...
    for model_name in selected_models:
        ...
        for iteration in iterations:
            # Load trained model (if not statistical)
            trained_model = None
            if not is_statistical:
                try:
                    trained_model = load_trained_model(model_name, iteration, models_dir)
                    print(f"\n✓ Loaded {model_name} iteration {iteration}")
                except FileNotFoundError as e:
                    print(f"\n❌ {e}")
                    continue
            
            # Predict for each file
            for file_info in tqdm(files_to_predict, desc=f"{model_name} iter{iteration}"):
                try:
                    # Load training data
